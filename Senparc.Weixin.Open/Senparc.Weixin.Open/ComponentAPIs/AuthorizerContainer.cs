...
namespace Senparc.Weixin.Open.ComponentAPIs
{
    ...
    public class AuthorizerContainer : BaseContainer<AuthorizerBag>
    {
        ...
        /// <returns></returns>
        public static AuthorizationInfo GetAuthorizationInfo(string componentAppId, string authorizerAppid, string refreshTokeninput = null,
            bool getNewTicket = false)
        {
            ...
            lock (authorizerBag.Lock)
            {
                ...
                if (getNewTicket || authorizerBag.AuthorizationInfoExpireTime <= DateTime.Now)
                {
                    ...
                    var refreshToken = string.IsNullOrWhiteSpace(refreshTokeninput) ? ComponentContainer.GetAuthorizerRefreshTokenFunc(authorizerAppid)
                        : refreshTokeninput;
